...
func RenderGuidelines(project core.ProjectConfig) string {
	...
	builder.WriteString("Disable it for one command with `--no-sync`, for this project with `autoSync`\n")
	builder.WriteString("in `.workbook/config.json`, or for every project with `\"autoSync\": false` in\n")
	builder.WriteString("the user configuration's `preferences` block. A project policy outranks a user\n")
	builder.WriteString("preference; `--no-sync` outranks both.\n\n")
